...
public class QueryableObjectPoolExtendedTests
{
    ...
    public void TestDisposalBehavior()
    {
        ...
        Assert.True(pool.GetObject(c => c.Make == "Ford").IsFailure);
    }
    ...
    public async Task TestAsyncOperationsWithDisposal()
    {
        ...
        // Assert
        Assert.True((await pool.GetObjectAsync()).IsFailure);
    ...
    public void TestPoolHealthImplementation()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            models.Add(pool.GetObject().Unwrap());
        }
    ...
    public async Task TestQueryablePoolAsyncWithTimeoutAndCancellation()
    {
        ...
        Assert.True((await pool.GetObjectAsync(timeout)).IsFailure);
    ...
    public async Task TestQueryableAsyncEventualSuccess()
    {
        ...
        Assert.Equal("Ford", result2.Unwrap().Unwrap().Make);

        // Clean up
        result2.Unwrap().Dispose();
    }
        ...
        for (int i = 0; i < 500; i++)
        {
            ...
            tasks.Add(Task.Run(() =>
            {
                try
                {
                    ...
                    if (pool.TryGetObject(c => c.Make == $"Make{makeIndex}", out var model).IsSuccess)
                    {
    ...
    public void TestQueryablePoolWithValidation()
    {
        ...
        citroen.Unwrap().Dispose(); // This should validate and reject the car

        // Get a Ford car and return it
        var ford = pool.GetObject(c => c.Make == "Ford");
        ford.Unwrap().Dispose(); // This should validate and keep the car
        ...
        anotherCitroen.Unwrap().Dispose(); // This will be rejected too
        ...
        Assert.True(pool.GetObject(c => c.Make == "Citroen").IsFailure);
    }
    ...
    public void TestRaceConditionInQueryOperation()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            tasks.Add(Task.Run(() =>
            {
                try
                {
                    return pool.TryGetObject(c => c.Make == "Ford", out var model).IsSuccess ? model : null;
                }
                catch
                {
                    return null;
    ...
    public void TestQueryablePoolPerformance()
    {
        ...
        // 2. Query by Make
        using (var obj = pool.GetObject(c => c.Make == "Make1").Unwrap())
        {
            // Just get and return
        }
        ...
        using (var obj = pool.GetObject(c => c is { Make: "Make1", Model: "Model50" }).Unwrap())
        {
