...
public class PoolWarmupTests
{
    ...
    {
        // Arrange
        var config = new PoolConfiguration<Car> { MaxPoolSize = 10 };
    ...
    {
        // Arrange
        var config = new PoolConfiguration<Car> { MaxPoolSize = 100 };
        var pool = new DynamicObjectPool<Car>(() => new Car("Test", "Model"), config);
    ...
    [Fact]
    public async Task WarmUpToPercentageAsync_InvalidPercentage_ReturnsFailure()
    {
        // Arrange
        var pool = new DynamicObjectPool<Car>(() => new Car("Test", "Model"));

        // Act & Assert
        var resultNegative = await pool.WarmUpToPercentageAsync(-10);
        Assert.True(resultNegative.IsFailure);

        var resultOver = await pool.WarmUpToPercentageAsync(150);
        Assert.True(resultOver.IsFailure);
    }
