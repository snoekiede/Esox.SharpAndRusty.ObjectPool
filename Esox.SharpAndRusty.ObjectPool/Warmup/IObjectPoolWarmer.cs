...
public interface IObjectPoolWarmer<T> where T : notnull
{
    ...
    Task<ExtendedResult<Unit,Error>> WarmUpAsync(int targetSize, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>A task representing the warm-up operation</returns>
    Task<ExtendedResult<Unit,Error>> WarmUpToPercentageAsync(double targetPercentage, CancellationToken cancellationToken = default);
