...
public static class EvictionExtensions
{
    ...
    extension(IServiceCollection services)
    {
        ...
        {
            var descriptor = services.FirstOrDefault(d => d.ServiceType == typeof(ObjectPoolBuilder<T>));
            if (descriptor?.ImplementationInstance is ObjectPoolBuilder<T> builder)
                builder.WithTimeToLive(timeToLive, evictionInterval);
            return services;
        }
            ...
            TimeSpan? evictionInterval = null) where T : class
        {
            var descriptor = services.FirstOrDefault(d => d.ServiceType == typeof(ObjectPoolBuilder<T>));
            if (descriptor?.ImplementationInstance is ObjectPoolBuilder<T> builder)
                builder.WithIdleTimeout(idleTimeout, evictionInterval);
            return services;
        }
            ...
            TimeSpan? evictionInterval = null) where T : class
        {
            var descriptor = services.FirstOrDefault(d => d.ServiceType == typeof(ObjectPoolBuilder<T>));
            if (descriptor?.ImplementationInstance is ObjectPoolBuilder<T> builder)
                builder.WithEviction(timeToLive, idleTimeout, evictionInterval);
            return services;
        }
            ...
            TimeSpan? evictionInterval = null) where T : class
        {
            var descriptor = services.FirstOrDefault(d => d.ServiceType == typeof(ObjectPoolBuilder<T>));
            if (descriptor?.ImplementationInstance is ObjectPoolBuilder<T> builder)
                builder.WithCustomEviction(evictionPredicate, evictionInterval);
            return services;
        }
    }
}
